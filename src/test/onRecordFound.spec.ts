      ...
      jsc.forall(recordArbWithSuccesses, jsc.integer(1, 100), function(
        ...
        const distScopes = httpDistUrls.map(
          ({ url, success }: { url: string; success: CheckResult }) => {
            ...
            const scopeHead = scope.head(url.endsWith("/") ? "/" : "");
            const scopeGet = scope.get(url.endsWith("/") ? "/" : "");
            ...
            if (success !== "error") {
              if (!disallowHead && success === "success") {
                ...
                if (disallowHead) {
                  ...
                  scopeHead.reply(success === "success" ? 405 : 400);
                } else {
                ...
                if (success === "success") {
                  scopeGet.reply(200, () => {
                    const s = new RandomStream(streamWaitTime);
          ...
          registryScope
            .put(
              ...
              (body: BrokenLinkAspect) => {
                ...
                const isDownloadUrlHttp = hasProtocol(downloadURL, "http");
                const isAccessUrlHttp = hasProtocol(accessURL, "http");

                const isDownloadUrlHttpSuccess =
                  isDownloadUrlHttp && successLookup[downloadURL] === "success";
                ...
                const isAccessURLHttpSuccess =
                  isAccessUrlHttp && successLookup[accessURL] === "success";
                ...
                const isDownloadUrlDefined =
                  ...
                  !downloadUri.scheme() ||
                  parseUriSafe(downloadURL).scheme().length === 0;

                const is404: boolean =
                  result === "broken" &&
                  ((isDownloadUrlHttp &&
                    successLookup[downloadURL] === "notfound") ||
                    (isDownloadUrlDefined &&
                      isAccessUrlHttp &&
                      successLookup[accessURL] === "notfound"));
        ...
        >(
          ...
          (retryCount: number) => {
            ...
            const allResultsArb = arbFlatMap<number, number[]>(
              count429Arb,
              count429s => arrayOfSizeArb(count429s + 1, failureCodeLengthArb),
        ...
        return jsc.assert(
          jsc.forall(
            ...
            (record: Record, { retryCount, allResults }: FailuresArbResult) => {
              ...
              const distScopes = urlsFromDataSet(record).map(url => {
                ...
                allResults.forEach((failureCodes, i) => {
                  failureCodes.forEach(failureCode => {
                    ...
                    scope.get(url.endsWith("/") ? "/" : "").reply(failureCode);
                  });
              ...
              return onRecordFound(record, registry, retryCount, 0)
                .then(() => {
                  registryScope.done();
                  distScopes.forEach(scope => scope.done());
                })
